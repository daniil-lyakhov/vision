...
for model, weight in MODELS:
    ...
    #for use_nncf, bc in [(True, False), (True, True)]:
        ...
        args = get_args_parser().parse_args(f"--data-path /home/dlyakhov/datasets/imagenet/ --model {model} --test-only --weights {weight} --batch 64 ".split() + nncf_flags)
        exc = ""

        try:
            logging = []
            command = f"--data-path /home/dlyakhov/datasets/imagenet/ --model {model} --test-only --weights {weight} --batch 64 ".split() + nncf_flags + logging
            command = [sys.executable] + [os.path.join(os.getcwd(), "train.py")] + command
            print(command)
            ...
            print(exc)
            result = -1
        results.append((model, weight, use_nncf, bc, result, exc))
